...
class Transport:
    ...
    def __init__(
        ...
        logging.info(
            f"train_eps: {train_eps}, sample_eps: {sample_eps}, model_type: {model_type}, path_type: {path_type}, loss_type: {loss_type}, SNR type: {snr_type}"
        )
    ...
    def noise_sample(self, x1):
        t, x0, x1 = self.sample(x1)
        t, xt, ut = self.path_sampler.plan(t, x0, x1)
        return xt

    def training_losses(self, model, x1, model_kwargs=None, t=None, x0=None,xt=None, ut=None):
        ...
        assert model_output.size() == (B, C, *xt.size()[2:])
        ...
        *,
        ...
        def _likelihood_drift(x, t, model, **model_kwargs):
            ...
            with th.enable_grad():
                ...
                #print('*****************0logp grad shape', logp_grad.shape)
                #print('*****************0logp grad', logp_grad)
                drift = self.drift(x, t, model, **model_kwargs)
                #print('*****************drift', drift.shape)
                #print(-drift, logp_grad)
        ...
        def _sample_fn(x, model, **model_kwargs):
            #print('********entering sample_fn')
            init_logp = th.zeros(x.size(0)).to(x)
            input = (x, init_logp)
            drift, delta_logp = _ode.sample(input, model, **model_kwargs)
            #print('3***** delta_logp', delta_logp)
            drift, delta_logp = drift[-1], delta_logp[-1]
            prior_logp = self.transport.prior_logp(drift)
            logp = prior_logp - delta_logp
            #print('drift sum', drift.sum())
            return logp, delta_logp, drift
